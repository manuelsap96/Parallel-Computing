...
__device__ __host__ void bits_split64(uint64_t input, uint64_t *left,
		uint64_t *right);
__device__ __host__ void bits_split(uint64_t input, uint64_t *left,
		uint64_t *right, int len);
__device__ __host__ void bits_set(uint64_t *data, int nr, uint64_t val);
__device__ __host__ void bits_copy(uint64_t src, uint64_t *dst, int src_offset,
		int dst_offset, int len);
__device__ __host__ void bits_print(uint64_t val);
__device__ __host__ void bits_print_grouped(uint64_t val, int group_size,
		int length);

__device__ __host__ uint64_t bits_cycle_left(uint64_t val, int shift, int size);
__device__ __host__ uint64_t bits_cycle_right(uint64_t val, int shift,
		int size);
__device__ __host__ uint64_t bits_permutate(uint64_t key, int*permutation,
		int length, int key_length);
...
__device__ __host__ void bits_print(uint64_t val) {
	for (int i = 63; i >= 0; i--) {
		printf("%lu ", bits_bit64(val, i));
		if (i % 8 == 0)
			printf("  ");
	}
}

__device__ __host__ uint64_t bits_cycle_left(uint64_t val, int shift,
		int size) {
	uint64_t tmp = 0;
...
__device__ __host__ uint64_t bits_permutate(uint64_t key, int*permutation,
	...
	for (int i = 0; i < length; i++) {
		bits_set(&result, i,
				bits_bit64(key, key_length - permutation[length - 1 - i]));
	}
	return result;
}
